...
public class Julia extends JPanel {
    ...
    private static final double REALMIN = -1.5;
    ...
    private Palette[] load(String fileName) {
        ...
        try {
            ...
            Logger.getLogger(Julia.class.getName()).log(Level.SEVERE, null, ex);
        }
        return color;
    }
    ...
    private void draw(Graphics2D g2d) {
        ...
        for (int i = 0; i < maxx; ++i) {
            for (int j = 0; j < maxy; ++j) {
                ...
                z = Complex.ofCartesian(((REALMAX - REALMIN) / (maxx)) * i + REALMIN, ((IMAGMAX - IMAGMIN) / (maxy)) * j + IMAGMIN);
                //iniciamos Z=-0.745+0.1i
                c = Complex.ofCartesian(-0.745, 0.1);
                ...
                Palette palette = getColor(clamp(cont));
                int r = palette.r();
                int g = palette.g();
                int b = palette.b();
